...
/** First two characters of the name – also works with emoji/umlauts. */
export function initial(name) {
    const chars = [...name.trim()].slice(0, 2).join('');

    return (chars || '?').toUpperCase();
}

/** Label in the circle: two letters, or the whole word for the own token. */
